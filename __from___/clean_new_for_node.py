...
import os # run node-red on anaconda at path ../../leaf_script
...
MODEL_DIR = r'C:\Users\user01\Desktop\mask_rcnn_find_area_pixel\area_leaf\logs\leaf20211206T0903' ## windows will is \

# Import Mask RCNN
#ต้องมีโฟเดอร์ mrcnn ใน leaf_script แทน
...
# ใส่ไฟล์ config_leaf_pc ใน ../../leaf_script
import config_leaf_pc          #$ for import class LEAFconfig
config = config_leaf_pc.LEAFConfig()   #
...
file_name_cant_detect = [] #เก็บชื่อไฟล์ที่ detectไม่ได้
...
#
  ...
  for i in range(len([name for name in os.listdir(DIR) if name.endswith('.jpg')])):
    ...
    # cv2_imshow(mask)                                                                                                                                                                                          #
    or_maskLab_and_maskS = cv2.bitwise_or(thresh_s_blured_at_hsv,mask)
